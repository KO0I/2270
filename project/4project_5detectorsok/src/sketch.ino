...
void loop(){
  ...
  if(!p.A & p.B & p.C & p.D & p.E) {    // Far Left or A
    PORTD &=~((1<<CC_L)|(1<<CC_R));
    PORTD |= ((1<<CW_L)|(1<<CW_R));
  ...
  if(p.A & !p.B & p.C & p.D & p.E) {   // Left or B
    PORTD &= ~((1<<CC_L)|(1<<CC_R)|(1<<CW_L));
    PORTD |=  (1<<CW_R);
    //Serial.println("B");
  }
  if(p.A & p.B & !p.C & p.D & p.E) {   // Center or C
    //PORTD = 0B00110000;
    PORTD &=~((1<<CW_L)|(1<<CC_R));
    PORTD |= ((1<<CC_L)|(1<<CW_R));
    //Serial.println("C");
  ...
  if(p.A & p.B & p.C & !p.D & p.E) {   // Right or D
    PORTD &= ~((1<<CW_L)|(1<<CC_R)|(1<<CW_R));
    PORTD |=  (1<<CC_L);
    //Serial.println("D");
  }
  if(p.A & p.B & p.C & p.D & !p.E) {  // Far Right or E
    PORTD &=~((1<<CW_L)|(1<<CW_R));
    PORTD |= ((1<<CC_L)|(1<<CC_R));
    //Serial.println("E");
  }
